...
// ⭐ TYPE DEFINITION FOR OUR NEW MODAL CONFIGURATION
...
export default function AdminUsers() {
  ...
  // ⭐ STATES FOR DRIVING THE TAILWIND CUSTOM MODAL
  ...
  const verifyAndFetchUsers = async () => {
    ...
    if (adminCheck?.company_id !== 3) {
      toast.error('Access Denied: Only Super Admins can manage system roles.')
  ...
  // 🚨 RE-ENGINEERED INDIVIDUAL PURGE EXECUTOR
  const executeUserAccountDelete = async (targetUser: User) => {
    setDeletingId(targetUser.id)
    try {
      if (targetUser.is_admin) {
        await supabase.from('admin_users').delete().eq('id', targetUser.id)
      }
      ...
      console.error(err)
      toast.error(`Purge Failed: ${err.message || 'Check structural requirements.'}`)
    } finally {
      setDeletingId(null)
    }
  }

  // TRIGGER MODAL FOR DELETING AN ACCOUNT
  const handleDeleteUserAccount = (targetUser: User) => {
    setModal({
      isOpen: true,
      title: 'Delete User Account',
      message: `Are you completely sure you want to permanently erase the user account record for "${targetUser.name || targetUser.email}"? This will instantly revoke all platform access privileges.`,
      confirmButtonText: '🗑️ Delete User',
      onConfirm: () => executeUserAccountDelete(targetUser)
    })
  }

  // 🚨 RE-ENGINEERED TABLE PURGE EXECUTOR
  ...
  // TRIGGER MODAL FOR GRANULAR TABLE MAINTENANCE BUTTONS
  const handleClearSpecificTable = (table: 'order_items' | 'payments' | 'orders' | 'customers') => {
    let warningMessage = ''
    if (table === 'order_items') warningMessage = "Wipe all individual line-items inside 'order_items'? This leaves parent orders intact but removes product data mapping contents."
    if (table === 'payments') warningMessage = "Wipe all records inside 'payments'? This clears your Paynow mock simulation history logs completely."
    if (table === 'orders') warningMessage = "Wipe all rows inside 'orders'? NOTE: This action will fail if 'order_items' or 'payments' tables still contain references to these records."
    if (table === 'customers') warningMessage = "Wipe all customer profile datasets? This clears all directory entities except your active Super Admin session account."

    setModal({
      isOpen: true,
      title: 'Database Security Confirmation',
      message: warningMessage,
      confirmButtonText: '💥 Confirm Purge',
      onConfirm: () => executeTableClear(table)
    })
  }
  ...
  return (
    <div className="pb-16 w-full max-w-6xl mx-auto px-4 pt-6 relative">
      ...
      {/* HEADER SECTION */}
      <div className="mb-6">
        <h1 className="text-2xl font-black text-gray-900 tracking-tight">Identity & System Maintenance</h1>
        <p className="text-xs text-gray-400 mt-0.5">Manage clearance records and handle isolated database testing routines.</p>
      </div>
                ...
                {editingUser?.id === user.id ? (
                  <>
                    <td className="p-0 sm:px-5 sm:py-3 flex flex-col sm:table-cell w-full">
                      <input type="text" value={editingUser.name} onChange={(e) => setEditingUser({ ...editingUser, name: e.target.value })} className="border rounded-lg px-2 py-1.5 text-xs w-full bg-gray-50 focus:outline-none" />
                    </td>
                    <td className="p-0 sm:px-5 sm:py-3 flex flex-col sm:table-cell w-full text-gray-400 text-xs truncate">
                      <span className="truncate block mt-1 sm:mt-0">{user.email}</span>
                    </td>
                    <td className="p-0 sm:px-5 sm:py-3 flex flex-col sm:table-cell w-full">
                      <input type="tel" value={editingUser.phone} onChange={(e) => setEditingUser({ ...editingUser, phone: e.target.value })} className="border rounded-lg px-2 py-1.5 text-xs w-full font-mono bg-gray-50 focus:outline-none" />
                    </td>
                    <td className="p-0 sm:px-5 sm:py-3 flex flex-col sm:table-cell w-full">
                      <input type="text" value={editingUser.location || ''} onChange={(e) => setEditingUser({ ...editingUser, location: e.target.value })} className="border rounded-lg px-2 py-1.5 text-xs w-full bg-gray-50 focus:outline-none" />
                    </td>
                    <td className="p-0 sm:px-5 sm:py-3 flex flex-col sm:table-cell w-full">
                  ...
                  <>
                    <td className="p-0 sm:px-5 sm:py-4 flex flex-col items-start sm:table-cell w-full">
                      <span className="sm:hidden text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Name:</span>
                      <span className="font-bold text-gray-900 truncate block">{user.name || '-'}</span>
                    </td>
                    <td className="p-0 sm:px-5 sm:py-4 flex flex-col items-start sm:table-cell w-full min-w-0">
                      <span className="sm:hidden text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Email:</span>
                      <span className="text-gray-500 text-[11px] sm:text-sm truncate block font-medium" title={user.email}>{user.email}</span>
                    </td>
                    <td className="p-0 sm:px-5 sm:py-4 flex flex-col items-start sm:table-cell w-full">
                      <span className="sm:hidden text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Phone:</span>
                      <span className="font-mono text-xs text-gray-600 truncate block">{user.phone || '-'}</span>
                    </td>
                    <td className="p-0 sm:px-5 sm:py-4 flex flex-col items-start sm:table-cell w-full min-w-0">
                      <span className="sm:hidden text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Location:</span>
                      <span className="text-xs text-gray-500 truncate block" title={user.location}>{user.location || '-'}</span>
                    </td>
                    <td className="p-0 sm:px-5 sm:py-4 flex flex-col items-start sm:table-cell w-full">
                      <span className="sm:hidden text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1">Clearance:</span>
                      ...
                      }`}>{user.role === 'super_admin' ? 'Super' : user.role === 'mineazy_admin' ? 'Mineazy' : user.role === 'farmeazy_admin' ? 'Farmeazy' : 'Customer'}</span>
                    </td>
      ...
      {/* ⭐ STYLISH INTERMEDIARY TAILWIND POPUP ALERT OVERLAY */}
      {modal.isOpen && (
          ...
          <div className="bg-white border border-gray-100 rounded-2xl max-w-sm w-full p-6 shadow-xl space-y-4 transform scale-100 transition-all duration-200">
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                ...
                className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white text-xs font-black uppercase tracking-wider rounded-xl transition-all shadow-xs"
              >
